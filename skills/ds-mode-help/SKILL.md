...
Off by default. When enabled, every visual one-pager generated by the
stamper also publishes a PNG to a **private GitHub repo owned by you**, and
DS Mode includes a tappable URL in the reply. On your phone, log into
GitHub once (Safari or the GitHub mobile app) and the image renders inline
in the repo — no one else can see it.
...
2. Creates a **private** repo at `<your-github-user>/ds-mode-mobile` (or
...
After setup, the publish step happens in the background after every
stamper screenshot. The git push takes ~1–3 seconds; the URL is
deterministic so DS Mode prints it immediately. By the time you read
the reply and tap the URL on your phone, the push has landed.
...
| `/ds-mode mobile status` | Print configuration and current state. |
| `/ds-mode mobile setup` | Re-run the wizard. |

Privacy properties:

- Repo is **private**. `https://github.com/<you>/ds-mode-mobile/blob/main/<file>.png`
  requires you to be logged in as that GitHub user to view.
- raw.githubusercontent.com URLs to private repos require an OAuth
  token, so DS Mode uses the `blob/main` URL pattern instead — works
  in any browser session you're authenticated in.
- Nothing publishes to public hosts. Your content stays in your account.
- The local clone at `$CLAUDE_CONFIG_DIR/ds-mode-mobile/` accumulates
  history. To clean up: `cd $CLAUDE_CONFIG_DIR/ds-mode-mobile && git
  rm <old-files> && git commit && git push`. (No automatic retention
  yet.)
...
- An active GitHub account with permission to create private repos
  (free tier is fine)

Failure modes:

- No `gh` or unauthenticated → setup wizard exits with a clear error.
- `git push` failure mid-session → image stays local; the URL 404s
  until the next successful push. Logged to
  `$CLAUDE_CONFIG_DIR/.ds-mode-mobile.log`.
